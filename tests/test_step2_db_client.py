...
from storage.db_client import test_connection, get_receipt_candidates_by_date, get_regpayment_candidates_by_date
...
print("\n3. get_regpayment_candidates_by_date()")
test_cents = -95000  # <-- change to a value in your regpayment table
test_date2 = date(2026, 4, 30)
regpayments = get_regpayment_candidates_by_date(test_date2, signed_cents=test_cents)
...
no_receipts = get_receipt_candidates_by_date(date(2026, 4, 30), 28, amount=Decimal("0.01"))
no_regpayments = get_regpayment_candidates_by_date(date(2026, 4, 30), signed_cents=1)
