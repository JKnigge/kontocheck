...
python tests/test_step4_matcher.py                       # baseline matcher regression (44 manual checks)
...
documents the **original** design before the two-pass matcher redesign — each
file carries an "OUTDATED" banner at the top. They are retained as a historical
record only. The current authoritative source for matching rules and status
semantics is `REDESIGN_PLAN.md` (status: Implemented). Code comments still
...
|---|---|---|
| `tests/test_matcher_helpers.py` | Pure helper unit tests (U1–U41): `_to_signed_cents`, `_strip_thinking`, `_has_brand_overlap`, `_parse_verdict`, `_parse_choice_verdict`, `_build_candidate_choice_prompt` | Yes — mocks config, ollama, db_client |
| `tests/test_matcher_branches.py` | Branch logic unit tests (U42–U79): `_choose_candidate`, `_gather_amount_match_candidates`, `_gather_name_only_candidates`, `match_all`, `_reconcile_conflicts` | Yes — mocks config, ollama, db_client |
...
| `tests/test_step4_matcher.py` | **Legacy script-mode** (NOT a pytest module). Baseline matcher regression (44 manual checks). Calls `sys.exit()` at module load. | Yes — mocks config, ollama, db_client |
| `tests/test_step5_report.py` | **Legacy script-mode** (NOT a pytest module). Report rendering regression (39 manual checks). Calls `sys.exit()` at module load. | Yes — synthetic MatchResults |
...
Tests marked `@pytest.mark.xfail` encode known defects from `MATCHER_REVIEW.md`. Each xfail test has a `reason` string referencing the defect ID (e.g. `L13`). When a fix lands, the corresponding xfail test should flip to passing. **Do not remove xfail markers** unless the underlying defect has been fixed in `pipeline/matcher.py`.

Current xfail map (1 test):

| Defect | Tests | Issue |
|---|---|---|
| L13 | 1 (U9) | `_strip_thinking` doesn't handle unclosed `мот` tags |

The following defects were resolved by the two-pass redesign and their xfail
markers removed:

| Defect | Tests | Resolution |
|---|---|---|
| H3 | 2 (U55, U74) | Deferred commitment + conflict flagging naturally resolves the tie. |
| L12 | 2 (U56, U62) | Empty issuer/reason candidates are now dropped before the LLM call in `_gather_amount_match_candidates` / `_gather_name_only_candidates`. |
| L15 | 1 (U19) | "Kartenzahlung" and other payment-method noise tokens are now in `_BRAND_NOISE_TOKENS`. |
