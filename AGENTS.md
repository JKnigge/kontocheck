...
Tests marked `@pytest.mark.xfail` encode known defects from `Design Docs/MATCHER_REVIEW.md`. Each xfail test has a `reason` string referencing the defect ID (e.g. `L13`). When a fix lands, the corresponding xfail test should flip to passing. **Do not remove xfail markers** unless the underlying defect has been fixed in `pipeline/matcher.py`.
